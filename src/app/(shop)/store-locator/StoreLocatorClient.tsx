...
function StoreCard({ store }: { store: Store }) {
  ...
  return (
    <div data-testid="store-card" className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
      {/* Header */}
      <div className="p-5 pb-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h3 data-testid="store-card-name" className="font-bold text-gray-900 dark:text-gray-100 text-base leading-tight">
              {store.name}
            </h3>
...
export default function StoreLocatorClient() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data } = await apiClient.get<{ success: boolean; data: Store[] }>("/api/stores");
  ...
  return (
    <main data-testid="store-locator-page" className="min-h-screen bg-gray-50 dark:bg-gray-950">
      {/* Hero */}
      <div className="bg-[#FCA311] text-white py-12 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-white/80 text-base mb-6">
            {loading ? "Find your nearest location" : `${stores.length} Prakash Supermarket locations across London`}
          </p>
          ...
          <div className="relative max-w-lg mx-auto">
            ...
            <input
              ...
              type="search"
              aria-label="Search stores by name, city or postcode"
              value={search}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          ...
          <div data-testid="stores-error" className="text-center py-20 text-red-500 dark:text-red-400">
            <AlertCircle className="h-10 w-10 mx-auto mb-3 opacity-60" />
            <p className="font-semibold text-lg">Could not load stores</p>
