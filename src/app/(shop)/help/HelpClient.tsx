...
function FaqItem({ faq }: { faq: Faq }) {
  ...
  return (
    <div data-testid="faq-item" className="border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden">
      <button
        id={questionId}
      ...
      >
        <span data-testid="faq-item-question" className="text-sm font-semibold text-gray-800 dark:text-gray-100 leading-snug">
          {faq.question}
        </span>
        {open
      ...
      {open && (
        <div
          ...
          className="px-5 pb-5 pt-1 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800"
        >
          <p data-testid="faq-item-answer" className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{faq.answer}</p>
        </div>
      )}
    </div>
  );
...
export default function HelpClient() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data } = await apiClient.get<{ success: boolean; data: Faq[] }>("/api/faqs");
  ...
  const isFiltering = active !== "all" || search.trim().length > 0;

  return (
    <main data-testid="help-page" className="min-h-screen bg-gray-50 dark:bg-gray-950">
      {/* Hero */}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          ...
          <div data-testid="faqs-error" className="text-center py-20 text-red-500 dark:text-red-400">
            <AlertCircle className="h-10 w-10 mx-auto mb-3 opacity-60" />
            <p className="font-semibold text-lg">Could not load FAQs</p>
          ...
          /* Flat list when searching or single-category selected */
          <div className="space-y-2">
            {filtered.map((faq) => <FaqItem key={faq._id} faq={faq} />)}
          </div>
        ) : (
          /* Grouped sections — ordered by CATEGORIES constant, not DB sort */
          <div className="space-y-8">
            {CATEGORIES.filter((c) => c.key !== "all" && grouped[c.key]).map(({ key, label, emoji }) => (
              <section key={key} aria-label={label}>
                <h2 className="flex items-center gap-2 text-base font-bold text-gray-800 dark:text-gray-200 mb-3">
                  <span>{emoji}</span> {label}
                </h2>
                <div className="space-y-2">
                  {grouped[key].map((faq) => <FaqItem key={faq._id} faq={faq} />)}
                </div>
              </section>
            ))}
          </div>
        ...
        {!loading && (
          <div className="mt-12 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 text-center">
            ...
            <a
              ...
              data-testid="contact-support-cta"
