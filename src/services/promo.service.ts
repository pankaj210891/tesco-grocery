...
function eligibleSubtotal(offer: LeanOffer, items: CartItemInput[]): number {
  ...
  return items
    .filter((i) => {
      const okCategory = !hasCategories || offer.eligibleCategories.includes(i.category);
      const okProduct  = !hasProducts   || offer.eligibleProductIds.includes(i.productId);
 ...
 */
export async function applyPromo(input: PromoApplyInput): Promise<PromoApplyResult> {
  await connectDB();
  ...
  if (!offer.isActive) {
    return { valid: false, discountAmount: 0, error: "This promo code is no longer active" };
  }
  if (new Date(offer.expiresAt) < new Date()) {
    return { valid: false, discountAmount: 0, error: "This promo code has expired" };
  }
  ...
  const [totalCount, userCount, firstOrderCheck] = await Promise.all([
    ...
    offer.perUserLimit != null && userId ? userUsageCount(code, userId)    : Promise.resolve(0),
    offer.firstOrderOnly && userId       ? hasAnyOrder(userId)             : Promise.resolve(false),
  ]);
  ...
  if (offer.perUserLimit != null && userId && userCount >= offer.perUserLimit) {
    return { valid: false, discountAmount: 0, error: "You have already used this promo code the maximum number of times" };
  }
  if (offer.firstOrderOnly && firstOrderCheck) {
    return { valid: false, discountAmount: 0, error: "This promo code is for first orders only" };
  }
  ...
  if (offer.eligiblePincodes.length > 0 && postcode) {
    ...
    const ok    = offer.eligiblePincodes.some((p) => upper.startsWith(p.trim().toUpperCase()));
    if (!ok) {
      return { valid: false, discountAmount: 0, error: "This promo code is not available for your delivery area" };
    }
  }

  // ── 6. Category / product eligibility ────────────────────────────────────────
  const eligSub = eligibleSubtotal(offer, items);
  const hasRestrictions = offer.eligibleCategories.length > 0 || offer.eligibleProductIds.length > 0;

  if (hasRestrictions && eligSub === 0) {
    const restriction = offer.eligibleCategories.length > 0
...
export async function getEligiblePromos(
  ...
  const isReturningCustomer  = needsFirstOrderCheck ? await hasAnyOrder(userId) : false;

  // Fetch per-user usage for all offers that have perUserLimit, in parallel
  const codeList = offers.filter((o) => o.perUserLimit != null).map((o) => o.code!);
  const userCounts: Record<string, number> = {};
  if (userId && codeList.length > 0) {
    const counts = await Promise.all(
      codeList.map((c) => userUsageCount(c, userId)),
    );
    codeList.forEach((c, i) => { userCounts[c] = counts[i]; });
  }

  // Fetch total usage for offers with usageLimit
  const limitedCodes = offers.filter((o) => o.usageLimit != null).map((o) => o.code!);
  const totalCounts: Record<string, number> = {};
  if (limitedCodes.length > 0) {
    const counts = await Promise.all(limitedCodes.map((c) => totalUsageCount(c)));
    limitedCodes.forEach((c, i) => { totalCounts[c] = counts[i]; });
  }
  ...
  for (const offer of offers) {
    ...
    // Usage limits
    if (offer.usageLimit != null && (totalCounts[code] ?? 0) >= offer.usageLimit) continue;
    if (offer.perUserLimit != null && userId && (userCounts[code] ?? 0) >= offer.perUserLimit) continue;
